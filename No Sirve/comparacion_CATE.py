...
X_boot = df_model[X_cols]
n_boot = 50
cate_means = []

for i in range(n_boot):
    idx = np.random.choice(len(X_boot), len(X_boot), replace=True)
    
    cate_boot = cf_model.effect(X_boot.iloc[idx])
    
    cate_means.append(np.mean(cate_boot))

cate_means = np.array(cate_means)

print("----- Bootstrap del CATE medio -----")
print("MEDIA BOOTSTRAP:", cate_means.mean())
print("STD BOOTSTRAP:", cate_means.std())
print("IC 95%:", np.percentile(cate_means, [2.5, 97.5]))


cate_means = []
for i in range(20):
    df_tr, df_te = train_test_split(df_model, test_size=0.3, random_state=i)
    
    cf_model.fit(
        Y=df_tr["satisfaccion"],
        T=df_tr["teletrabajo"],
        X=df_tr[X_cols],
        W=df_tr[W_cols]
    )
    
    cate = cf_model.effect(df_te[X_cols])
    cate_means.append(np.mean(cate))

print("----- Estabilidad por submuestras -----")
print("MEDIA:", np.mean(cate_means))
print("STD:", np.std(cate_means))


importances = []
for i in range(10):
    idx = np.random.choice(len(df_model), len(df_model), replace=True)
    
    cf_model.fit(
        Y=df_model.iloc[idx]["satisfaccion"],
        T=df_model.iloc[idx]["teletrabajo"],
        X=df_model.iloc[idx][X_cols],
        W=df_model.iloc[idx][W_cols]
    )
    
    importances.append(cf_model.feature_importances_)

importances = np.array(importances)

print("----- Estabilidad del ranking de variables -----")
print("IMPORTANCE MEAN:", importances.mean(axis=0))
print("IMPORTANCE STD:", importances.std(axis=0))