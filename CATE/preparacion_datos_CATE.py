...
#####    "n_jbterm1": "contrato_perm",

    "n_isced11_dv": "educacion",

    "n_sclfsat2": "satisfaccion_ingresos",

###    "n_mlstat": "estado_civil",
...
###    "n_basrate":  "salario",
    "n_jbot": "horas_extra",
###    "n_jbotpd": "horas_extra_pagadas"

    "n_jbttwt": "tiempo_viaje_trabajo",
###    "n_wktrvfar": "modo_transporte"
...
# TIPO DE CONTRATO (1 fijo, 0 temporal)
#####df_model["contrato_perm"] = df_model["contrato_perm"].apply(
...
def recode_marital(x):
    if x in [1]:
        ...
        return 2 # en relacion
    elif x in [4, 5, 6, 7, 8, 9]:
        return 3 # separado_divorciado_viudo
    else:
        return np.nan

#df_model["estado_civil"] = df_model["estado_civil"].apply(recode_marital)
...
###df_model["estres_laboral"] = df_model[estres_cols].mean(axis=1, skipna=True)
###df_model = df_model.drop(columns=estres_cols)
...
# MODO DE TRANSPORTE
# 0 = transporte activo (bicicleta, caminar)
# 1 = transporte privado (coche, moto)
# 2 = transporte público (bus, tren, metro)
###df_model["modo_transporte"] = df_model["modo_transporte"].apply(
###    lambda x:
###        1 if x in [1, 2, 3, 4, 5] else   # privado
###        2 if x in [6, 7, 8] else         # público
###        0 if x in [9, 10] else           # activo
###        np.nan
###)
